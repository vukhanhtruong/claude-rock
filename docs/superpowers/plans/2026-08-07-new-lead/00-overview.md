...
- **Nesting depth means ESLint `max-depth` semantics.** `else if` continuations in a flat `if / else if / else` chain are one level, not one level per branch. Counting them naively inflates the depth and produces spurious gate breaches (settled at final review, triage item 54).
- **`assets/dashboard/index.html` and `assets/dashboard/detail.html` are exempt from the 200-line gate** (652 and 853 lines). Each page is self-contained by design, and splitting one into HTML + CSS + JS means three files copied into every leads root that must version-stamp in lockstep or a refresh half-updates a root. Stated honestly: that cost is not prohibitive — `stats.mjs` and `vendor/reactflow-bundle.js` are already separately copied, separately stamped files and `index.html` imports `stats.mjs` across that boundary successfully — so this is a decision to pay the lockstep cost twice and not a third time, not a claim that it cannot be done. The ≤ 20-lines/function, ≤ 3-params and ≤ 2-nesting gates still apply inside both pages.
- **`scripts/lib/map.mjs` is exempt from the user's global 10-functions-per-file gate** (20 functions, 187 lines; ruling H7). Not because merging the helpers is impossible — the file has three clean seams (read sources / build the node-edge graph + layout / build panels) and would split into three files of 6-8 functions with no function changing at all. The reason is that `map.mjs` is in `init-root.mjs`'s `SCRIPT_FILES` and is copied into every leads root: each extra file is another version stamp to keep in lockstep and another surface on the refresh mechanism, which finding I3 showed is fragile in exactly that way. Same reasoning as the dashboard-page exemption above.
