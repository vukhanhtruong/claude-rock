...
- Static: **allowlist only** — `/stats.mjs`, `/vendor/*`, and `/<lead-id>/dist/*` (`^[a-z0-9-]+$` ids). Resolve `const p = resolve(root, '.' + decodeURIComponent(url.pathname));` — 403 when `!p.startsWith(root + sep)` or any segment starts with `.`; 404 for anything outside the allowlist (notably `/leads.json`, `/serve.mjs`, `/lib/*`, `/<lead>/notes.md`, `/<lead>/new-lead-answers.json`, `/<lead>/brief.md` — that content reaches the UI only via the API); 404 when missing; content-type from `MIME[extname]` fallback `application/octet-stream`. Tests must assert: `/stats.mjs` and `/vendor/reactflow-bundle.js` 200; `/leads.json` and `/acme-crm/notes.md` 404.
